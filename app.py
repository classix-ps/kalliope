...
classificationModel = AutoModelForSequenceClassification.from_pretrained(f"../qlora/output/distilbert-base-uncased-classification-instruct10/checkpoint-80000")
...
def query(type):
    if flask.request.method == 'POST':
        ...
        if type == "name":
            classifier = pipeline("text-classification", model=classificationModel, tokenizer=classificationTokenizer)
        ...
        return flask.make_response(flask.jsonify({"output": response}), 200)
